...
exports.checkAndCancelExpiredSlotBookings = checkAndCancelExpiredSlotBookings;
exports.initSlotBookingCron = initSlotBookingCron;
 ...
 */
async function checkAndCancelExpiredSlotBookings() {
    try {
        ...
        const expiredBookings = await prisma_1.default.slotBooking.findMany({
            where: {
                status: 'PENDING',
                createdAt: {
        ...
        for (const booking of expiredBookings) {
            ...
            const updated = await prisma_1.default.slotBooking.update({
                where: { id: booking.id },
                data: {
                    status: 'REJECTED',
            ...
            const slotField = booking.vehicleType === 'TWO_WHEELER'
                ? 'availableSlotsTwoWheeler'
                : 'availableSlotsCar';
            const dailySlotField = booking.vehicleType === 'TWO_WHEELER'
                ? 'dailySlotsTwoWheeler'
                : 'dailySlotsCar';
            // Get current center data
            const center = await prisma_1.default.center.findUnique({
            ...
            if (center) {
                ...
                if (currentAvailable < dailyLimit) {
                    await prisma_1.default.center.update({
                        where: { id: booking.centerId },
                        data: {
                            [slotField]: {
                                increment: 1,
                    ...
                    console.log(`[Slot Auto-Cancel] Skipping slot restoration - already at daily limit (${currentAvailable}/${dailyLimit})`);
                }
            }
            console.log(`[Slot Auto-Cancel] Cancelled booking ${booking.id} - ${booking.vehicleNumber} (Customer: ${booking.customerMobile}) - NO_SHOW`);
...
function initSlotBookingCron() {
    ...
    console.log(`[Slot Auto-Cancel] Cron job initialized - will check every 30 minutes for bookings older than ${AUTO_CANCEL_TIMEOUT_HOURS} hours`);
...
//# sourceMappingURL=slotBooking.service.js.map